...
首次使用时，脚本会要求选择工作模式。添加、编辑或删除规则只会改变待应用列表，必须在菜单中选择“应用并保存”才会生效。

### 编辑已有规则

在交互菜单选择“编辑转发规则”，再选择已有规则的序号。脚本会依次显示名称、协议、入口网卡、中转端口、目标地址、目标端口、来源限制和 TCP MSS；每个提示直接回车表示保留当前值。

例如，只为已有 TCP 规则启用自动 MSS：前面的字段全部直接回车，在 `TCP MSS` 提示处输入 `auto`，检查汇总后输入 `SAVE`，最后回到主菜单选择“应用并保存”。
